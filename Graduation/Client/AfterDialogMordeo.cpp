#include "stdafx.h"
#include "AfterDialogMordeo.h"

#include "After_DialogBox.h"
#include "AfterLaily.h"

CAfterDialogMordeo::CAfterDialogMordeo()
{
}

CAfterDialogMordeo::~CAfterDialogMordeo()
{
	Release();
}

HRESULT CAfterDialogMordeo::Initialize()
{
	if (FAILED(CTextureMgr::GetInstance()->InsertTexture(L"../Texture/Dialog/Dialog_Mordeo.png", L"DiaMordeoBack", TEX_SINGLE)))
	{
		ERR_MSG(L"Dialog Background Image Insert Failed!!!");
		return E_FAIL;
	}

	CObj* pMordeoLaily = nullptr;
	if (FAILED(CAbstractFactory<CAfterLaily>::CreateObj(pMordeoLaily)))
		return E_FAIL;
	CObjMgr::GetInstance()->AddObject(pMordeoLaily, OBJ_DIALOG_MATIKA);

	CObj* pAfterDialog = nullptr;

	if (FAILED(CAbstractFactory<CAfter_DialogBox>::CreateObj(pAfterDialog)))
		return E_FAIL;


	CObjMgr::GetInstance()->AddObject(pAfterDialog, OBJ_DIALOGBOX);

	m_pAfterDialog = dynamic_cast<CAfter_DialogBox*>(pAfterDialog);



	return S_OK;
}

void CAfterDialogMordeo::Update()
{
	CObjMgr::GetInstance()->Update();
}

void CAfterDialogMordeo::LateUpdate()
{
	CObjMgr::GetInstance()->LateUpdate();

	if (CKeyMgr::GetInstance()->KeyDown(KEY_SPACE))
	{
		++iAfterDiaCnt;

		m_pAfterDialog->SetAfterDiaCnt(iAfterDiaCnt);

		if (m_pAfterDialog->GetAfterDiaCnt() > 4)
		{
			CSceneMgr::GetInstance()->SceneChange(CSceneMgr::STAGE2_BOSS);
			return;
		}
	}

	if (CKeyMgr::GetInstance()->KeyDown(KEY_RETURN))
	{
		CSceneMgr::GetInstance()->SceneChange(CSceneMgr::STAGE2_BOSS);
		return;
	}
}

void CAfterDialogMordeo::Render()
{
	const TEXINFO* pTexInfo = CTextureMgr::GetInstance()->GetTexture(L"DiaMordeoBack");

	D3DXMATRIX matScale, matRotZ, matTrans, matWorld;

	D3DXMatrixScaling(&matScale, 1.f, 1.f, 1.f);

	D3DXMatrixTranslation(&matTrans, (WINCX / 2) - CScrollMgr::GetScroll().x, (WINCY / 2) - CScrollMgr::GetScroll().y, 0.f);

	matWorld = matScale * matTrans;

	//// SetTransform: 행렬을 반영하는 함수.
	CDevice::GetInstance()->GetSprite()->SetTransform(&matWorld);

	// 이미지의 중점 구하기.
	int iCenterX = pTexInfo->tImgInfo.Width / 2;
	int iCenterY = pTexInfo->tImgInfo.Height / 2;

	//// 앞으로 모든 오브젝트의 렌더링은 이 사이에서 진행.
	CDevice::GetInstance()->GetSprite()->Draw(pTexInfo->pTexture,
		nullptr /*출력할 이미지 영역의 RECT*/,
		&D3DXVECTOR3((float)iCenterX, (float)iCenterY, 0.f) /*출력할 이미지의 중심 좌표 D3DXVECTOR3*/,
		nullptr /*출력할 위치 좌표 D3DXVECTOR3*/,
		D3DCOLOR_ARGB(255, 255, 255, 255));

	CObjMgr::GetInstance()->Render();
}

void CAfterDialogMordeo::Release()
{
	CTextureMgr::GetInstance()->DestroyInstance();
	CObjMgr::GetInstance()->DestroyInstance();
}
